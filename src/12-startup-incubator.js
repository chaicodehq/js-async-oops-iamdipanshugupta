...
export class Incubator {
  ...
  async admitStartup(startup) {
    ...
    this.#startups.push({
      startup,admittedAt:new Date().toISOString(),
      demoCompleted:false
    });
  ...
  async batchProcess(startups) {
    ...
    return results.map(result => {
      if(result.status === "fulfilled")
        ...
        return {status:"rejected" , resaon: result.reason.message}
      }
    })
  }
...
export async function runDemoDay(incubator) {
  ...
  const demoPrmoises = startups.map(startup => incubator.conductDemo(startup.name));
  const settled = await Promise.allSettled(demoPrmoises);

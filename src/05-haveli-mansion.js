...
export class HaveliSecurity {
  ...
  removeResident(name, passcode) {
    ...
    const residentIndex = this.#residents.findIndex(resident => resident.name === name);
    if(residentIndex === -1){
  ...
  verifyAccess(name) {
    const isResident = this.#residents.some(resident => resident.name === name);
    const accessEntry = { name, time: new Date().toISOString(), allowed: isResident };
